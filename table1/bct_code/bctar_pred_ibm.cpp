...
long double beta = 0.75; // prior parameter
const long double alpha = pow((1.0 - beta), (1.0 / (m - 1.0)));
const short k_max = 1;

const short ar_p = 1; //order of AR models
//const double e_n = 0.1346; // variance of Gaussian noise, i.e. en=sigma^2


double mu_0;       //mean vector of Gaussian prior for parameters
double sigma_0; // cov matrix of Gaussian prior for parameters, det these to values at start of main

long double tau = 0.1;
long double lambda = 50.0;
...
const vector<double> init_s2(ar_p, 0);//used to initialize sums s2 and s3 to all zero vector and matrix
const vector < vector <double> > init_s3(ar_p, init_s2);
...
int main() {
	...
	//Mean vector m0 here is just scalar since the AR order in the differenced series is p=1, which implies p=2 for the original series 
	mu_0 = 0.00;
	sigma_0 = 1.0;
	...
	ifstream if_file("ibm.txt", ios::in);
...
void insert(tree& T, vector <short> ct2, short ch) {
	...
	for (int i = 0; i < ar_p; i++) { init->s2.push_back(0); }
	for (int i = 0; i < ar_p; i++) {
...
void p_est(node* N) { // this calculates the estimated probability N->le from the sums
	...
	double s2; //store matrices for eigen-library
	double s3;//this is symmetric, positive definite (or semi-definite, but probably iinvertible)
	double s3_inv;//inverse matrix
	double identit;//identity matrix
	...
	long double A_B = N->s1 + mu_0 * mu_0 / sigma_0;
	double t = s2 + mu_0 / sigma_0;
	double S2 = s3 + 1.0 / sigma_0;


	double Sigma = 1.0 / S2; //this is S2.inverse() but numerically stable


	A_B = A_B - t * t / S2; // A_B is A+B
	...
	double J = s3 * sigma_0 + I_matrix;
	lp = lp - 0.5 * log2(J) - 0.5 * N->Bs * log2(2.0 * M_PI);
	...
	for (int d = 0; d < D + 1; d++) {
		...
		for (int k = 0; k < T[d].size(); k++) {
			...
			if (T[d][k]->leaf == 1) {
				...
				double Sigma_inv = s3 + 1.0 / sigma_0; //not in equations exactly like this



				double Sigma = 1.0 / Sigma_inv; //this is Sigma_inv.inverse() but numerically stable



				double m = 1.0 / Sigma_inv * (mu_0 / sigma_0 + s2); // mean

				long double A_B = N->s1 + mu_0 * mu_0 / sigma_0;
				double t = s2 + mu_0 / sigma_0;
				//Matrix<double, ar_p+1, ar_p+1> S2 = s3 + sigma_0.inverse();

				A_B = A_B - t * m; // A_B is A+B
...
long double predict_one_step(tree T, vector <short> ct, vector<double> x_tilde) { //x is real value used for log-loss, function used in predict_online using T_map
	...
	double mean = (1.0 / Sigma_inv) * (mu_0 / sigma_0 + s2); // mean

	long double A_B = N->s1 + mu_0 * mu_0 / sigma_0;
	double t = s2 + mu_0 / sigma_0;
	//Matrix<double, ar_p, ar_p> S2 = s3 + sigma_0.inverse(); // this is Sigma_inv

	A_B = A_B - t * t / Sigma_inv; // A_B is A+B


	double nu = 2 * tau + 1.0 * N->Bs;

	long double y = mean * x_tilde[0];  // only for ar_p=1
...
void pred_mse(vector <double> xn, int train_size) { //this calculates log-loss exactly as discrete case, using full predictive distribution (not possible in reality to achieve this)
	...
	for (int i = train_size; i < xn.size() - 4; i++) { /////-4 is for MULTISTEP
		...
		//long double y0 = predict_multi_step(trees[0], ct, x_tilde, 5, 10000);
		...
		//cout << "mse2 is " << mse2 << endl;
	}

	cout << endl;
	cout << "mse is " << mse / xn.size() * 2 << endl;
...
long double predict_multi_step(tree T, vector <short> ct, vector<double> x_tilde, short step, int N_samples) {
	...
	for (int n_traj = 0; n_traj < N_samples; n_traj++) {
		...
		for (int n_step = 0; n_step < step; n_step++) {
			...
			long double m_current = (1.0 / Sigma_inv) * (mu_0 / sigma_0 + s2) * x_tildec[0]; ////THIS ONLY for ar_p=1.

			//for (int k = 0; k <  1; k++) {
				//if (m(k)> 0.05 || m(k)<-0.05) {
				//m_current = m_current + m*x_tildec[k];
				//}
			//}
			...
			x_tildec[0] = sample; // this is only for ar_p=1
