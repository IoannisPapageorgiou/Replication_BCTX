...
Matrix<double, ar_p, 1> mu_0;       //mean vector of Gaussian prior for parameters
Matrix<double, ar_p, ar_p> sigma_0; // cov matrix of Gaussian prior for parameters, det these to values at start of main
...
long double predict_one_step(tree T, vector <short> ct, double x, vector<double> x_tilde, long double& mse);
...
default_random_engine generator(4663);

int main() {

	sigma_0 = sigma_0.Identity(); // for prior hyperparameters
	sigma_0 = 1.0 * sigma_0;


	vector <double> xn;

	ifstream if_file("sim1.txt", ios::in);
	...
	pred_mse(xn, xn.size()/2); return 0;
...
void p_est(node* N) { // this calculates the estimated probability N->le from the sums
	...
	Matrix<double, ar_p, 1> s2; //store matrices for eigen-library
	Matrix<double, ar_p, ar_p> s3;//this is symmetric, positive definite (or semi-definite, but probably iinvertible)
	Matrix<double, ar_p, ar_p> s3_inv;//inverse matrix
	Matrix<double, ar_p, ar_p> identit;//identity matrix
	...
	Matrix<double, ar_p, 1> t = s2 + sigma_0.inverse() * mu_0;
	Matrix<double, ar_p, ar_p> S2 = s3 + sigma_0.inverse();
	...
	Matrix<double, ar_p, ar_p> I_matrix;
	I_matrix = I_matrix.Identity();

	Matrix<double, ar_p, ar_p> J = s3 * sigma_0 + I_matrix;
	...
	for (int d = 0; d < D + 1; d++) {
		...
		for (int k = 0; k < T[d].size(); k++) {
			...
			if (T[d][k]->leaf == 1) {
				...
				Matrix<double, ar_p, ar_p> Sigma_inv = s3 + sigma_0.inverse(); //not in equations exactly like this
				...
				Matrix<double, ar_p, 1> m = Sigma_inv.inverse() * (sigma_0.inverse() * mu_0 + s2); // mean
				...
				//here code for likelihood using the MAP parameters (i.e. similar to ML really)

				long double sigma_map = (2.0 * lambda + A_B) / (2.0 * tau + 1.0 * N->Bs + 2.0);
				long double log_L = -0.5 * N->Bs * log(2.0 * M_PI * sigma_map) - 0.5 * (N->s1 - 2.0 * m.transpose() * s2 + m.transpose() * s3 * m) / sigma_map;


				cout << "ln-likelihood is  " << log_L << endl;

				cout << "Bs/2 is " << 0.5 * N->Bs << endl;
				cout << "Ds/2 is " << 0.5 * A_B << endl;

				for (int kl = 0; kl < N->s.size(); kl++) {
					cout << N->s[kl];
				}
				cout << endl;
				//Matrix<double, ar_p , 1> x_n;
				//x_n(0) = -38; x_n(2) = 1;
				//cout << x_n.transpose() * Sigma * x_n << endl;
...
void log_loss(vector <double> xn, int train_size) { //this calculates log-loss exactly as discrete case, using full predictive distribution (not possible in reality to achieve this)
	...
	for (int i = D; i < train_size; i++) {
		...
		vector<double> x_tilde(ar_p);     //continuous context needed for sums
		for (int j = 0; j < ar_p; j++) {
			x_tilde[j] = xn[i - j - 1];
		}
...
long double predict_one_step(tree T, vector <short> ct, double x, vector<double> x_tilde, long double& mse) { //x is real value used for log-loss, function used in predict_online using T_map
...
void pred_mse(vector <double> xn, int train_size) { //this calculates log-loss exactly as discrete case, using full predictive distribution (not possible in reality to achieve this)
	...
	for (int i = train_size; i < xn.size(); i++) {
		...
		long double y = predict_one_step(trees[0], ct, s, x_tilde, mse);
