...
public:
	...
	vector <double> theta = init_s2; //current ML estimate
	//double sigma = 0; // sigma_s at time n
	//vector <double> dsigma; // derivative of sigma at time n
	//vector <double> dL; //derivtive of log-lik
	//vector < vector <double> > d2L; // hessian
...
default_random_engine generator(4663000);
...
void occur(node* N, double s, vector<double> x_tilde) {
	...
	for (int i = 0; i < ar_p + 1; i++) {
		x2s.push_back(x_tilde[i] * x_tilde[i]);
	}
	...
	double sigma2 = 0;
	for (int i = 0; i < ar_p + 1; i++) {
		sigma2 = sigma2 + N->theta[i] * x2s[i]; // ths is sigma SQUARED
	}

	if (sigma2 == 0) {
		N->theta[0] = 1.0 / (ar_p + 1);
		sigma2 = N->theta[0];
		//cout << "geia" << endl;
	}

	N->s1 = N->s1 - 0.5 * log(2.0 * M_PI) - 0.5 * log(sigma2) - 0.5 * s * s / sigma2; //log-lik (lne)

	for (int i = 0; i < ar_p + 1; i++) {
		N->s2[i] = N->s2[i] + 0.5 * (s * s / sigma2 - 1.0) * x2s[i] / sigma2; //gradient of log-lik
	}


	for (int i = 0; i < ar_p + 1; i++) {
		for (int j = 0; j < ar_p + 1; j++) {
			N->s3[i][j] = N->s3[i][j] + 0.5 * x2s[i] * x2s[j] / sigma2 / sigma2; //hessian of loglik
		}
	}
...
void p_est(node* N) { // this calculates the estimated probability N->le from the sums
	...
	if (N->Bs > ar_p) {
		...
		N->le = 1.5 * log2(2.0 * M_PI) - 0.5 * log2(s3.determinant()) + N->s1 / log(2.0) - log2(N->theta[0]); // - log2(N->theta[0]) for prior

	}

	else {
		N->le = 0; //avoid numerical problems for inversion of s3 matrix - and its determinant is 0.
		cout << "SKIP" << endl;
	}

	//cout << N->Bs << endl;
	//cout << N->le << endl;
	//cout << log2(s3.determinant()) << endl;
	//cout << log2(N->theta[0]) << endl;
	//cout << N->s1 << endl;
}
...
void fisher_it(node* N) {
	...
	Matrix<double, ar_p + 1, 1> s2; //store matrices for eigen-library
	Matrix<double, ar_p + 1, ar_p + 1> s3;//this is symmetric, positive definite (or semi-definite, but probably iinvertible)
	Matrix<double, ar_p + 1, 1> delta_thetas;

	//N->s2[0] = N->s2[0] - 1.0 / N->theta[0]; // for MAP instead of ML
	//N->s3[0][0] = N->s3[0][0] + 1.0 / N->theta[0] / N->theta[0]; // for MAP intead of ML
	...
	Matrix<double, ar_p + 1, ar_p + 1> s3_inv = s3.inverse();//inverse matrix

	delta_thetas = s3_inv * s2;

	//cout << N->Bs << endl;
	//cout << s3.eigenvalues() << endl;
	//cout << s3 << endl;
	//cout << s3_inv << endl;
	//cout << s3_inv * s3 << endl;
	...
	for (int i = 0; i < ar_p + 1; i++) {
		...
		//cout << N->theta[i] << "  ";
		N->theta[i] = N->theta[i] + delta_thetas(i, 0);
		...
		//cout << delta_thetas(i, 0) << "  ";
	//	cout << N->theta[i] << "  ";

	}
	//cout << endl;
	//cout << s3.determinant() << endl;
}
...
long double predict_one_step(tree T, vector <short> ct, vector<double> x_tilde) { //x is real value used for log-loss, function used in predict_online using T_map
	...
	for (int k = 0; k < ar_p + 1; k++) {
		...
		y = y + N->theta[k] * x_tilde[k] * x_tilde[k];
		//}
	}
...
void pred_mse(vector <double> xn, int train_size) { //this calculates log-loss exactly as discrete case, using full predictive distribution (not possible in reality to achieve this)
	...
	for (int M = 0; M < 10; M++) { //fisher iterations
		...
		for (int i = D; i < xn.size(); i++) {
			...
			for (int j = 0; j < D; j++) { //here use QUANTISED version for disrete context
				...
				//if ((xn[i - j - 1] > -0.1666) && (xn[i - j - 1] < 0.0833)) {
				//	ct[j] = 1;
				//}
				//if ((xn[i - j - 1] <= -0.1666) ) {
				//	ct[j] = 0;
		...
		for (int d = 0; d < D + 1; d++) {
			for (int k = 0; k < T[d].size(); k++) {
				node* N_c = T[d][k];
				fisher_it(T[d][k]); // update theta_ML
				p_est(T[d][k]);  // compute laplace approx of Pes with these theta estimates 

				if (M < 9) {
					T[d][k]->Bs = 0; //reset all the counts for new pass of fisher algorithm, but not the thetas
					T[d][k]->s1 = 0;
					T[d][k]->s2 = init_s2;
					T[d][k]->s3 = init_s3;
				}
	...
	for (int i = train_size; i < xn.size(); i++) { /////-4 is for MULTISTEP
		...
		//cout << endl;
		//cout << trees[0][1].size() << endl; tree  T_pr;
		//if (trees[0][1].size() > 0) {
		//	T_pr = trees[0];
		//}
		//else { T_pr = trees[1]; }
...
//now this is the variance												  //long double y2 = predict_one_step(trees[2], ct, x_tilde);



//long double y0 = predict_multi_step(trees[0], ct, x_tilde, 1, 100000);
//long double y1 = predict_multi_step(trees[1], ct, x_tilde, 4, 10000);
//long double y2 = predict_multi_step(trees[2], ct, x_tilde, 4, 10000);

//long double y = c0 / c_norm * y0 + c1 / c_norm * y1 + c2 /c_norm * y2;
//mse2 = mse2 + (y - xn[i + 3]) * (y - xn[i + 3]);
		mse = mse + (y0 - xn[i + 0] * xn[i + 0]) * (y0 - xn[i + 0] * xn[i + 0]);
		ll = ll - 0.5 * log(2 * M_PI * y0) - 0.5 * (xn[i + 0] * xn[i + 0]) / y0;
	...
	cout << endl;
	cout << "The log-predictive density is " << ll << endl;
